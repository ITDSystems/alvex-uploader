// TODO
// solve mysterious problem with length/length()

// TODO - Alfresco reports incorrect js if this import is included, so as workaround resolveNode() was just copied from there.
   ...
   try
   {
      ...
      for each (field in formdata.fields)
      {
         switch (String(field.name).toLowerCase())
         {
		  ...
		  // remove incorrect symbols from file name
		  var incorrectSymbols = '"*\><?/:|]';
		  for (i = 0; i < incorrectSymbols.length; i++)
			  filename = filename.replace(incorrectSymbols[i], '_');
            ...
            case "thumbnails":
               ...
               break;
			case "createuploaddirectory":
			   createUploadDirectory = field.value == "true";
			   break;
         }
      }
      ...
      else if (destination !== null)
      {
          ...
          */
         destNode = resolveNode(destination);
         if (destNode === null)
         {
            status.code = 404;
      ...
      else
      {
         ...
         if (uploadDirectory !== null && uploadDirectory.length() > 0)
         {
			...
			else if (createUploadDirectory)
				// create upload directory
				destNode = createFolders(destNode, uploadDirectory);
            if (destNode === null)
            {
               status.code = 404;
         ...
         if (existingFile !== null)
         {
            ...
            if (existingFile.hasAspect("cm:versionable") && overwrite)
            {
               ...
               if (contentType !== null)
               {
                  existingFile.specializeType(contentType);
               }
               existingFile.save();
