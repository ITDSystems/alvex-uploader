...
(function()
{
	...
	YAHOO.extend(Alvex.Uploader, Alfresco.component.Base,
	{
		...
		*/
		...
		{
			var noContentMode = (this.options.useRegistries 
					&& ! this.options.useLocalDrive 
					&& ! this.options.useRepo);
			if ( (this.id != param[1]) && !noContentMode )
			{
				this.initUiWithFiles( true, Dom.get(param[1]+'-cntrl-current').value.split(',') );
				var refs = Dom.get(param[1]+'-cntrl-current').value.split(',');
				for(var i = 0; i < refs.length; i++)
					this.addRefToControl( this.id + "-cntrl-added", refs[i] );
		...
		checkParentUploader: function ()
		{
			...
			this.options.isInDocRegForm = Alvex.util.isInDocRegForm(this.id);
			// If relation was configured manually - just exis
			if( (this.options.uploaderId != null) || (this.options.parentUploaderId != null) )
				return;
			// Skip by default uploaders that work with metadata-only objects.
			// It is required, for instance, to grab files from workflow into newly created document
			// without creating a mess when they go into 'related documents' section.
			// If you need to pass metadata-only objects - please, configure them manually.
			var noContentMode = (this.options.useRegistries 
					&& ! this.options.useLocalDrive 
					&& ! this.options.useRepo);
			// If nothing was configured - set defaults
			if( (this.options.isInRelatedWorkflowForm || this.options.isInDocRegForm) && ! noContentMode ) {
